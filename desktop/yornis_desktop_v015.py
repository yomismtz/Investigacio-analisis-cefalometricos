...
# DPI awareness must be enabled before the first Tk root is created. This keeps
# Segoe UI text crisp instead of letting Windows bitmap-scale the application.
import yornis_hidpi_v0152 as hidpi
hidpi.enable_process_dpi_awareness()
...
# Clinical geometry audit stays active in both modes.
import yornis_clinical_audit_v0151
# Adds C1–C7 lordosis, guided cervical posture and CVM CS1–CS6.
...
class Launcher(base.Launcher):
    def __init__(self):
        yornis_theme.apply_theme(yornis_theme.current_theme_name(), persist=False)
        super().__init__()
        self.title("Yornis · Yom Dental Análisis")
        self.title_lbl.config(text="Yornis")
        self.ver_lbl.config(text=f"Yom Dental Análisis · v{APP_VERSION}")
        self._add_yornis_panel()
        hidpi.configure_window(self, role="launcher")
    ...
    def refresh_text(self):
        ...
        if hasattr(self, "note"):
            self.note.config(
                text=self.tr(
                    "Ciencia que vuela más alto · análisis cefalométrico e investigación.",
                    "Science that flies higher · cephalometric analysis and research.",
                )
            )
    ...
    def _add_yornis_panel(self):
        ...
        ttk.Label(row, text=self.tr("Tema inspirado en aves:", "Bird-inspired theme:")).pack(side="left", padx=4)
        cb = ttk.Combobox(row, state="readonly", values=yornis_theme.theme_names(), width=20)
        cb.set(yornis_theme.current_theme_name())
        cb.pack(side="left")
        cb.bind("<<ComboboxSelected>>", lambda e: self._theme(cb.get()))

        why = ttk.LabelFrame(self.main, text=self.tr("¿Por qué Yornis?", "Why Yornis?"), padding=6)
        why.pack(fill="x", padx=20, pady=4)
        for head, text in [
            ("Y", "Yom"),
            ("ORNIS", self.tr("Ave · raíz clásica", "Bird · classical root")),
            ("VUELO", self.tr("Precisión y visión", "Precision and vision")),
            ("CIENCIA", self.tr("Tecnología que inspira", "Technology that inspires")),
        ]:
            f = ttk.Frame(why, padding=5)
            f.pack(side="left", fill="both", expand=True, padx=3)
            ttk.Label(f, text=head, font=("Segoe UI", 10, "bold")).pack()
            ttk.Label(f, text=text, wraplength=160).pack()

    def _theme(self, name):
        yornis_theme.apply_theme(name)
        self.note.config(
            text=self.tr(
                f"Tema {name} seleccionado. Se aplicará a Yornis.",
                f"{name} theme selected. It will apply to Yornis.",
            )
        )
...
def research_app(lang):
    ...
    # Install last so the cervical panel sees the final trace/menu/calculation wrappers.
    ...
    hidpi.configure_window(app, role="research")
    return app
...
def main():
    ...
    if mode == "individual":
        ...
        hidpi.configure_window(app, role="individual")
        app.mainloop()
    elif mode == "research":
